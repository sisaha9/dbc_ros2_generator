...
out_dispatch_file = (
    out_pkg_can_dir / "include" / "pkg_name_can".replace("pkg_name", pkg_name) / "dispatch.hpp"
)
out_can_header = (
    out_pkg_can_dir
    / "include"
    / "pkg_name_can".replace("pkg_name", pkg_name)
    / "pkg_name_can.hpp".replace("pkg_name", pkg_name)
)
...
def replace_package_name(folder_name):
    for path_object in folder_name.glob("**/*"):
        if path_object.is_file():
            ...
            path_object_text = path_object_text.replace(
                "PkgName", pkg_name.replace("_", " ").title().replace(" ", "")
            )
...
def build_dataclasses(can_dbc, dbc_dict):
    ...
    for msg in can_dbc.messages:
        dbc_msg = DbcMessage(msg.name, "", "", f"0x{msg.frame_id:004x}", dbc_dict[msg.name], [])
...
def create_new_package():
    ...
    for path_object in out_pkg_dir.glob("**/*"):
        path_object.rename(
            Path(
                path_object.parent,
                path_object.stem.replace("pkg_name", pkg_name) + path_object.suffix,
            )
        )
...
def make_ros_messages(dbc_msgs, msg_template, out_msg_dir, out_msg_cmake, ros_msg_dir):
    ...
    for idx, ros_msg in enumerate(ros_msgs):
        ros_msg_name = (
            "".join([dbc_msg_part.capitalize() for dbc_msg_part in dbc_msgs[idx].name.split("_")])
            + ".msg"
        )
    ...
    cmake_out_msg = cmake_out_msg.replace(
        "MSG_FILES",
        "\n".join(
            ["\t" + '"' + ros_msg_dir + ros_msg_name + '"' for ros_msg_name in ros_msg_names]
        ).strip(),
    )
...
def modify_dispatch(dbc_msgs, out_dispatch_file):
    dispatch_ids = ""
    for dbc_msg in dbc_msgs:
        dispatch_msg_name = "\tID_" + "_".join(
            [dbc_msg_part.upper() for dbc_msg_part in dbc_msg.name.split("_")]
        )
...
def modify_header(dbc_msgs, out_can_header):
    ...
    recv_can_msgs = ""
    recv_can_msgs_template = (
        "\tvoid recvCANFUNC(const Frame::SharedPtr msg, DbcMessage * message);\n"
    )
    recv_ros_msgs = ""
...
def modify_source(dbc_msgs, out_can_source):
    ...
    ros_publishers_initialize_template = (
        '\tpubROSMSG_ = this->create_publisher<ROSMSG>("MSGNAME", rclcpp::SensorDataQoS());\n'
    )
    ...
    for dbc_msg in dbc_msgs:
        if "publish" in dbc_msg.comm_type:
            ros_publishers_initialize += ros_publishers_initialize_template.replace(
                "ROSMSG", dbc_msg.ros_name
            ).replace("MSGNAME", dbc_msg.name.lower())
            switch_case_id += switch_case_recv_id_template.replace(
                "SWITCHID", dbc_msg.dispatch_id_name
            ).replace("ROSMSG", dbc_msg.ros_name)
            recv_can_body_signal = ""
            for signal in dbc_msg.signals:
                recv_can_body_signal += recv_can_body_signal_template.replace(
                    "FIELDNAME", signal.name.lower()
                ).replace("SIGNAME", signal.name)
            recv_can_body_signal = recv_can_body_signal.strip()
            recv_can_body += recv_can_body_template.replace("ROSMSG", dbc_msg.ros_name).replace(
                "FILL_SIGNALS", recv_can_body_signal
            )
        if "receive" in dbc_msg.comm_type:
            ros_subscribers_initialize += ros_subscribers_initialize_template.replace(
                "ROSMSG", dbc_msg.ros_name
            ).replace("MSGNAME", dbc_msg.name.lower())
            recv_ros_body_field = ""
            for signal in dbc_msg.signals:
                recv_ros_body_field += recv_ros_body_field_template.replace(
                    "FIELDNAME", signal.name.lower()
                ).replace("SIGNAME", signal.name)
            recv_ros_body_field = recv_ros_body_field.strip()
            recv_ros_body += (
                recv_ros_body_template.replace("ROSMSG", dbc_msg.ros_name)
                .replace("DISPATCH_ID", dbc_msg.dispatch_id_name)
                .replace("FILLSIGNALS", recv_ros_body_field)
            )
    ...
    out_can_source_text = out_can_source_text.replace(
        "ROS_PUBLISHERS_INITIALIZE", ros_publishers_initialize
    )
    out_can_source_text = out_can_source_text.replace(
        "ROS_SUBSCRIBERS_INITIALIZE", ros_subscribers_initialize
    )
...
def reformat_pkg(pkg_dir):
    ...
    subprocess.run(["black", "-l", "99", str(pkg_dir)], shell=True)
    subprocess.run(["ament_uncrustify", "--reformat", str(pkg_dir)], shell=True)
